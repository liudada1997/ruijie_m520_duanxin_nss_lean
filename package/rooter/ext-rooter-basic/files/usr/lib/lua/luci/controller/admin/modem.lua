...
function index()
	entry({"admin", "modem"}, firstchild(), "模块功能", 35).dependent=false
--	entry({"admin", "modem", "cinfo"}, cbi("rooter/connection", {autoapply=true}), "Connection Info", 10)
	entry({"admin", "modem", "prof"}, cbi("rooter/profiles"), "连接配置文件", 2)
--	entry({"admin", "modem", "conmon"}, cbi("rooter/connmonitor"), "Connection Monitoring", 20)
	entry({"admin", "modem", "nets"}, template("rooter/net_status"), "网络状态", 30)
	entry({"admin", "modem", "debug"}, template("rooter/debug"), "调试信息", 50)
	entry({"admin", "modem", "cust"}, cbi("rooter/customize"), "自定义模块端口", 55)
	entry({"admin", "modem", "log"}, template("rooter/log"), "连接日志", 60)
	entry({"admin", "modem", "misc"}, template("rooter/misc"), "模块其他功能", 40)
