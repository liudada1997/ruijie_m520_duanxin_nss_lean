...
static void* udhcpc_thread_function(void*  arg) {
    ...
    if (profile->IPType == 0x04)
    {
        ...
        //snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "busybox udhcpc -b -s dhcpc-event -H Router -i %s", (char *)ifname);
        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "busybox udhcpc -b -H Router -i %s", (char *)ifname);
    }
    else
    {
    /*
