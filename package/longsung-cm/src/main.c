...
#define POLL_DATA_CALL_STATE_SECONDS 0//15 //poll data call state, for qmi ind maybe not work well

int debug_qmi = 1;  //modified by chenlei 
int main_loop = 0;
int g_command = 3;  //added by chenlei
char * qmichannel;
...
//added by chenlei for select device at 20161215 begin
typedef struct _STU_DEVICE_PATH
{
	int flag;
	char path[128];
}STU_DEVICE_PATH;
static STU_DEVICE_PATH s_stDevicePath = { 0, ""};
//added by chenlei for select device at 20161215 end
...
static int usage(const char *progname) {
     ...
     dbg_time("Usage: %s [-s [apn [user password auth]]] [-f logfilename] ", progname);
     ...
     dbg_time("Example 3: %s -s 3gnet carl 1234 0 -f gobinet_log.txt", progname);
     //dbg_time("Example 4: %s -c 1", progname);
     return 0;
}
    ...
    while ((ent = readdir(pDir)) != NULL) 
	{
        if ((strncmp(ent->d_name, "cdc-wdm", strlen("cdc-wdm")) == 0) || (strncmp(ent->d_name, "qcqmi", strlen("qcqmi")) == 0)) 
		{
            ...
            *pp_qmichannel = (char *)malloc(32);
            ...
            {
	            sprintf(net_path, "/sys/class/net/%s", s_stDevicePath.path);
            }
			//added by chenlei for select device at 20161215 end
...
//#ifdef ANDROID
//int longsung_CM(int argc, char *argv[])
//#else
...
#define has_more_argv() ((opt < argc) && (argv[opt][0] != '-'))
            ...
            case 'd':
                if (has_more_argv())
                {
                	if (NULL != strcpy(s_stDevicePath.path, argv[opt++]))
                	s_stDevicePath.flag = 1;                  
                }
                else
                {
                	s_stDevicePath.flag = 0;
                }
            break;
			//added by chenlei for select device at 20161215 end
            ...
            case 'c':
	            if (has_more_argv())
	            {
					g_command = atoi(argv[opt++]);
					dbg_time("g_command=%d\n", g_command);
	            }
            break;
    ...
    dbg_time("Longsung_ConnectManager_V01R01C01");
...
__main_loop:
    while (!profile.qmichannel)
    {
        ...
        if (main_loop)
        {
            ...
            dbg_time("Wait for Longsung modules connect");
        ...
        dbg_time("Cannot find qmichannel(%s) usbnet_adapter(%s) for Longsung modules", profile.qmichannel, profile.usbnet_adapter);
        return -ENODEV;
    }
...
//     else
	...
	{
		int res = 0;
		res = requestSetupDataCall(&profile);
		dbg_time("%s: requestSetupDataCall, res=%d\n", __func__, res);

		
		res = requestGetIPAddress(&profile);
		dbg_time("%s: requestGetIPAddress, res=%d\n", __func__, res);
	...
	if (g_command >= 0 && g_command <= 2)
	{
		...
		case 2:
			...
			break;
		default:
			dbg_time("Param error!\n");
			break;
		}
		
		//sleep(3);
		pthread_cancel(gQmiThreadID);
		goto __main_force_exit;
	}
#endif
        ...
        for (ne = 0; ne < nevents; ne++) {
                    ...
                    switch (signo)
                    {
                        ...
                        case SIGUSR2:
							...
							if (profile.ipv4.Address != 0) 
							{
								...
								ConnectionStatus = QWDS_PKT_DATA_DISCONNECTED;
								requestDeactivateDefaultPDP();
							}
							//usbnet_link_change(0, &profile);
							dbg_time("Disconnected!\n");
                        ...
                        case SIGHUP:
                        case SIGINT:
							dbg_time("%s SIGTERM/SIGHUP/SIGINT\n", __func__);
                            if (QWDS_PKT_DATA_CONNECTED == ConnectionStatus)
                                requestDeactivateDefaultPDP();
                            usbnet_link_change(0, &profile);
                            if (!strncmp(profile.qmichannel, "/dev/cdc-wdm", strlen("/dev/cdc-wdm")))
            ...
            if (fd == qmidevice_control_fd[0]) {
                if (read(fd, &triger_event, sizeof(triger_event)) == sizeof(triger_event)) {
					dbg_time("%s triger_event=0x%x\n", __func__, triger_event);
                    switch (triger_event) {
                        ...
                        #else 
							dbg_time("%s RIL_UNSOL_DATA_CALL_LIST_CHANGED reported\n", __func__);
                        #endif
                        }
                        break;
...
__main_quit:
    usbnet_link_change(0, &profile);
__main_force_exit:
	dbg_time("%s starting join gQmiThreadID\n", __func__);
    if (pthread_join(gQmiThreadID, NULL)) {
        dbg_time("%s Error joining to listener thread (%s)", __func__, strerror(errno));
    }
